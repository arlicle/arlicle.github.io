...
function animateLoading() {
    const messages = CONFIG.encouragingMessages;
    const messageDuration = 1500; // 每条消息显示1.5秒
    let messageIndex = 0;
    ...
    const loadingInterval = setInterval(() => {
        ...
        elements.loadingMessage.textContent = messages[messageIndex];
        // 重新触发动画
        elements.loadingMessage.style.animation = 'none';
        elements.loadingMessage.offsetHeight; // 强制重绘
        elements.loadingMessage.style.animation = 'messageSlide 1.5s ease-in-out';
    }, messageDuration);

    // 总加载时间 = 消息数量 × 每条消息时长
    const totalLoadingTime = messages.length * messageDuration;
...
function handleTileClick(e) {
    ...
    // 如果触摸滑动已执行过，忽略此次点击
    if (touchSwipePerformed) {
        touchSwipePerformed = false; // 重置标志
        return;
    }
...
let touchSwipePerformed = false; // 标记是否执行了触摸滑动，用于阻止后续的 click 事件
...
function handleTouchStart(e) {
    ...
    if (target && target.classList.contains('tile')) {
        ...
        touchSwipePerformed = false; // 重置滑动标记
...
function handleTouchEnd(e) {
    ...
    if (isDragging && dragDirection && distance >= swapThreshold) {
        ...
        if (targetRow >= 0 && targetRow < CONFIG.gridSize &&
            targetCol >= 0 && targetCol < CONFIG.gridSize) {
            // 标记已执行滑动操作，阻止后续 click 事件
            touchSwipePerformed = true;
