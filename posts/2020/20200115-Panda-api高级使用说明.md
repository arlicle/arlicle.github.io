...
`$exclude:["category/id", tags/0/id]` , 我们会得到下面的数据:
...
```
如果我们`$ref`了`Article`, 并且要重写`category`对象的`id`属性, 同时为`catebory`对象增加一个`order`, 为`tags`对象增加一个`order`, 可以这么写:
```.language-json5
"catebory/id":{name:"新的分类id", type:"PosInt"},
"catebory/order":{name:"分类排序", type:"int"},
...
...
    ...
    title: {name:"文章标题"},
    category: {
        id:{name:"新的分类id", type:"PosInt"},
        name:{name:"分类名称", type:"string"},
        order:{name:"分类排序", type:"int"}
    },
    tags:[{
        id:{name:"标签id"},
        title:{name:"标签内容"},
        order:{name:"标签排序", type:"int"}
    }]
...
```
