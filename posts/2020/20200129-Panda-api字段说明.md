...
- `max_length` 输出`map`的最大长度，`max_length`默认为7（选填）
    ...
    menus:[
        {
            ...
            children:{
                ...
                // 还可以指定固定长度的length
                min_layer:0, // 递归生成的最小层数
                max_layer:3, // 递归生成的最大层数
                // 还可以指定固定长度的layer
            }
        }
    ]
...
- `$ref`: `$ref`的值表示从哪一个节点开始递归,使用`$response`表示`response`模型的根节点，然后往下，属性之间用`/`分隔
- `name`: `rec` 的名称
- `desc`: `rec` 的说明
- `length` 输出数组的长度，`length`默认为0（选填）
- `min_length` 输出数组的最小长度，`min_length`默认为0（选填）
- `max_length` 输出数组的最大长度，`max_length`默认为7（选填）
- `layer_count` 递归多少层，默认为0（选填）
- `min_layer_count` 递归最小层，默认为0（选填）
- `mac_layer_count` 递归最大层，默认为3（选填）
