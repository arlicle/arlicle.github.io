...
```.language-json5
money: {name: "Moeny", type: "float", decimal_places:2},
```
mock 生成的小数就会只保留两位小数

- `min_decimal_places` 表示小数的随机的最小位数，默认为2位
```.language-json5
money: {name: "Moeny", type: "float", min_decimal_places:2},
```

- `max_decimal_places` 表示小数的随机位数的最大位数， 默认最大10位
```.language-json5
money: {name: "Moeny", type: "float", max_decimal_places:10},
```


通常`min_decimal_places`和`max_decimal_places`会同时使用，例如
```.language-json5
money: {name: "Moeny", type: "int", min_value:1, min_decimal_places:2, max_decimal_places:10},
```
